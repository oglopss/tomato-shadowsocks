...
libsodium_build()
{
    ...
    sed -e '/checking for thread local storage (TLS) class/,+34 s/^/# /' -i ./configure 
    sed -e '/checking for thread local storage (TLS) class/,+34 s/^/# /' -i ./configure.status
    ...
    sed -e '/checking for thread local storage (TLS) class/a ac_cv_tls=none' -i ./configure.status
